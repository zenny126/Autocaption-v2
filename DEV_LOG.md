...
## [2026-07-15] Tích hợp tính năng Thuyết minh bằng XTTS-v2 (Zero-shot Voice Cloning)

* **Tác vụ**: Thêm tính năng chuyển phụ đề SRT thành giọng thuyết minh tiếng Việt thông qua mô hình clone giọng nói cục bộ XTTS-v2.
* **Người thực hiện**: AI Agent (Antigravity)
* **Các file thay đổi**:
  * [requirements.txt](file:///d:/Project/Autocaption-v2/requirements.txt): Cập nhật dependencies (`coqui-tts`, `torchcodec`, `transformers==4.46.3`, `pydub`, `srt`).
  * [src/core/config.py](file:///d:/Project/Autocaption-v2/src/core/config.py): Thêm đuôi `.srt` vào `SUPPORTED_EXTS` để hỗ trợ kéo thả tệp phụ đề.
  * [src/workers/tts_worker.py](file:///d:/Project/Autocaption-v2/src/workers/tts_worker.py): Tạo mới worker thread xử lý nạp model XTTS-v2, tổng hợp giọng nói, tự động co giãn tốc độ đọc bằng FFmpeg `atempo` và ghép nối thành file thuyết minh hoàn chỉnh.
  * [src/ui/main_window.py](file:///d:/Project/Autocaption-v2/src/ui/main_window.py): Chuyển đổi giao diện sang dạng 2 Tab ("Tạo phụ đề" và "SRT -> Giọng nói"), hỗ trợ tự động nhận dạng kéo thả thông minh cho từng tab, kết nối các sự kiện với `TTSWorker`.
  * [src/assets/AutoCaption.css](file:///d:/Project/Autocaption-v2/src/assets/AutoCaption.css): Bổ sung CSS dark mode cho `QTabWidget` và `QTabBar` với dải màu tím nhấn của AutoCaption.
* **Mô tả chi tiết**:
  * Khắc phục triệt để lỗi import liên quan đến `isin_mps_friendly` và `is_torch_greater_or_equal` do phiên bản `transformers` 5.x không tương thích bằng cách vá trực tiếp code `coqui-tts` và ghim phiên bản `transformers==4.46.3`, cài thêm `torchcodec` để tương thích PyTorch 2.x.
  * Thiết kế tính năng thuyết minh chạy cục bộ (Offline) 100% sử dụng mô hình XTTS-v2 (~1.8GB) cho phép người dùng tự do nạp tệp âm thanh ghi âm giọng mẫu ngắn 6-10s để thuyết minh toàn bộ tệp phụ đề Việt theo giọng đó.
  * Tích hợp cơ chế tự động phát hiện độ dài câu nói, nếu dài hơn thời gian hiển thị phụ đề gốc sẽ tự động dùng FFmpeg tăng tốc độ đọc (tối đa 2.0x) để bảo đảm giọng đọc không bị lệch múi giờ.
  * Hoàn thành kiểm thử cú pháp và chạy ứng dụng thực tế thành công trên Windows.
